 ...
 * @author chandra
 */
public class AudioSenderPipeline extends Pipeline{
    ...
    public long streamTo(int roomId) {
        ...
        AudioSenderBin room = new AudioSenderBin(SENDER_ROOM_PREFIX + roomId,
    ...
    public void stopStreamingToRoom(int roomId) {
        ((AudioSenderBin) getElementByName(SENDER_ROOM_PREFIX + roomId)).getOut();
    }

    
    public void streamTo(String ip, int port) {
        // create the sender bin
        unicastSender = new AudioSenderBin(SENDER_UNICAST, ip, port, false);
        // add it to this
        add(unicastSender);
